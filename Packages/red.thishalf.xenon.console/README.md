...
This package is still very, very much Work In Progress. Don't expect anything to actually work yet.


## TODO
- Create the Console Terminal GameObjects in a separate scene
	- also load it as early as possible (maybe on splash screen if possible)
- Argument Parsing
	- Probably ditch the way of parsing using a sub class of `CommandArgumentType` in favor of the `ConsoleArgumentParser` attribute
	- Implement a check for `ConsoleArgumentParser`s to ensure they take a string as the only parameter and the return type is the same as enclosing type
	- Prioritise a method with a `ConsoleArgumentParser` attribute over searching for an explicit cast from a string
	- Cache (and prefetch or lazyinit) the methods with `ConsoleArgumentParser` attribute for better performance (also for explicit casts)
		- Maybe allow for a `ConsoleArgumentParser` method to specify whether it is safe to cache with a optional bool (could be useful with some weird runtime generated types)
	- Find a way to cleanly handle native type parsing when using `ConsoleArgumentParser` way of parsing
- Command Fetching
	- Find a more efficient way to scan for command methods than looping through everything
	- Handle the loading of assemblies later than the init (probably using AppDomain.AssemblyLoad event)
- Console Terminal
	- Create the UI
	- Implement a command history
		- Allow navigating through the history with up/down arrow keys
	- Implement message display
		- Have a bridge to display messages logged using the Xenon.Log class
		- Have a bridge to display messages logged through Unity's Debug.Log methods
		- Implement a filter to select the messages seen
			- some filters for every level of unity log messages
			- some filters for every level of xenon log messages
			- some filters for xenon log message categories
			- filters should apply on already logged messages (without completely deleting them of course)
			- maybe log everything and only display messages that match the filter (could be slow in perf)
			- or give even more control by giving a second parallel filter that is used for saving/catching/logging messages
				- this filter could be automatically updated so that anything activated in the display filter also gets activated in the logging filter
				- or with a toggle made to exactly match the display filter
	- Find a robust way to detect the keypress for opening/closing the terminal window (maybe runtime generated InputSystem)
		- also provide the ability to customise the key





(Kept the template README.md bellow for reference)
# Package Starter Kit
...
<root>
  ...
  ├── Editor
  │   ├── Undefined.XenonConsole.Editor.asmdef
  │   └── EditorExample.cs
  ├── Runtime
  │   ├── Undefined.XenonConsole.asmdef
  │   └── RuntimeExample.cs
  ├── Tests
  │   ├── .tests.json
  │   ├── Editor
  │   │   ├── Undefined.XenonConsole.Editor.Tests.asmdef
  │   │   └── EditorExampleTest.cs
  │   └── Runtime
  │        ├── Undefined.XenonConsole.Tests.asmdef
...
1. Enter your package name. The name you choose should contain your default organization followed by the name you typed. For example: `Undefined.XenonConsole`.
...
To use the Inspector, select the `package.json` file in the Project browser. The **Package Xenon Console Manifest** page opens for editing.
...
* Store Editor-specific code under a root editor assembly definition file:
  ...
  `Editor/Undefined.XenonConsole.Editor.asmdef`

* Store runtime-specific code under a root runtime assembly definition file:

  `Runtime/Undefined.XenonConsole.asmdef`

* Configure related test assemblies for your editor and runtime scripts:

  `Tests/Editor/Undefined.XenonConsole.Editor.Tests.asmdef`

  `Tests/Runtime/Undefined.XenonConsole.Tests.asmdef`
...
Use the `Documentations~/Xenon Console.md` documentation file to create preliminary, high-level documentation. This document should introduce users to the features and sample files included in your package.  Your package documentation files will be used to generate online and local docs, available from the Package Manager UI.
